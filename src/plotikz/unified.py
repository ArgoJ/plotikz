...
def to_tikz(
    ...
    """
    Convert a Plotly figure (go.Figure/dict/list) or Matplotlib figure/axes to LaTeX/TikZ PGFPlots code.

    Parameters:
    -----------
    fig : Plotly Figure (go.Figure/dict/list), Matplotlib Figure/Axes, or None (default: active PyPlot figure)
    ...
    if fig is None:
        return from_pyplot(
            filename=str_filename, standalone=standalone, tsv_threshold=tsv_threshold
        )
    ...
    if isinstance(fig, (matplotlib.figure.Figure, matplotlib.axes.Axes)):
        return MatplotlibToTikz().to_tikz(
            fig, filename=str_filename, standalone=standalone, tsv_threshold=tsv_threshold
        )

    # 3. Check for Plotly Figure object, dict, or trace list via isinstance
